...
class App
    ...
    def list_book
        if !@@books.empty?
            @@books.each_with_index do |(book, index), i|
    ...
    def list_people
        if !@@people.empty?
          @@people.each_with_index do |(person, index), i|
            if person.class == Student
              puts "#{i + 1}) (Student) Name: #{person.name}, ID: #{person.id}, Age: #{person.age}"
            elsif person.class == Teacher
              puts "#{i + 1}) (Teacher) Name: #{person.name}, ID: #{person.id}, Age: #{person.age}, Specialization: #{person.specialization}"
            end
          end
        else
